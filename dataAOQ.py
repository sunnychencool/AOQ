...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab, opt, offcandipath=None, offcandisize=None):
        self.vocab = vocab
        loc = data_path + '/'
        ...
        # get offline hard negative candidates
        if data_split == 'train':
    ...
    def __getitem__(self, index):
        ...
        #print(caption, self.captions[int(i2thn[img_id][0]*5)], self.captions[int(i2thn[img_id][0]*5)+1])
        ...
        if self.split == 'train':
          ...
          tokens = nltk.tokenize.word_tokenize(
            str(captionhn).lower().decode('utf-8'))
          captionhn = []
          captionhn.append(vocab('<start>'))
          captionhn.extend([vocab(token) for token in tokens])
          captionhn.append(vocab('<end>'))
          targethn = torch.Tensor(captionhn)
          ...
          tokens = nltk.tokenize.word_tokenize(
            str(captionhn2).lower().decode('utf-8'))
          captionhn2 = []
          captionhn2.append(vocab('<start>'))
          captionhn2.extend([vocab(token) for token in tokens])
          captionhn2.append(vocab('<end>'))
          targethn2 = torch.Tensor(captionhn2)
        ...
        return image, target, index, img_id, caption_label, caption_mask, imagehn, targethn, imagehn2, targethn2
...
def collate_fn(data):
    ...
    images, captions, ids, img_ids, caption_labels, caption_masks, imageshn, captionshn, imageshn2, captionshn2 = zip(*data)
    ...
    return images, targets, lengths, ids, caption_labels_, caption_masks_, imageshn, targetshn, lengthshn, imageshn2, targetshn2, lengthshn2
...
def get_precomp_loader(data_path, data_split, vocab, opt, offcandipath=None, offcandisize=None, batch_size=100,
    ...
    """Returns torch.utils.data.DataLoader for custom coco dataset."""
    if data_split == 'train':
        dset = PrecompDataset(data_path, data_split, vocab, opt, offcandipath, offcandisize)
    else:
...
def get_loaders(data_name, vocab, offcandipath, offcandisize, crop_size, batch_size, workers, opt):
