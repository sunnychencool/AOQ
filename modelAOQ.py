...
class ContrastiveLoss(nn.Module):
    ...
    def __init__(self, marginonline=0.2, marginoffline=0, alpha = 0.3, beta = 1.5, measure=False, max_violation=False):
    ...
    def forward(self, im, s, imhnoff, shnoff, imhnoff2, shnoff2):
...
class VSRN(object):
    ...
    def __init__(self, opt):
        ...
        # Loss and Optimizer
        self.criterion = ContrastiveLoss(marginonline=opt.marginonline,
    ...
    def state_dict(self):
        state_dict = [self.img_enc.state_dict(), self.txt_enc.state_dict(), self.caption_model.state_dict()]
        return state_dict
    ...
    def forward_loss(self, img_emb, cap_emb, img_embhn, cap_embhn, img_embhn2, cap_embhn2, **kwargs):
        """Compute the loss given pairs of image and caption embeddings
        """
        loss = self.criterion(img_emb, cap_emb, img_embhn, cap_embhn, img_embhn2, cap_embhn2)
        # self.logger.update('Le', loss.data[0], img_emb.size(0))
        self.logger.update('Le_retrieval', loss.data, img_emb.size(0))
        return loss

    def train_emb(self, images, captions, lengths, ids, caption_labels, caption_masks, imageshn, captionshn, lengthshn, imageshn2, captionshn2, lengthshn2, *args):
        ...
        img_embhn, cap_embhn, GCN_img_emdhn = self.forward_emb(imageshn, captionshn, lengthshn)
        img_embhn2, cap_embhn2, GCN_img_emdhn2 = self.forward_emb(imageshn2, captionshn2, lengthshn2)
        ...
        self.optimizer.zero_grad()
        retrieval_loss = self.forward_loss(img_emb, cap_emb, img_embhn, cap_embhn, img_embhn2, cap_embhn2)
