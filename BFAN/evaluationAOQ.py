...
from dataAOQ import PrecompDataset
import time
import numpy as np
from vocab import Vocabulary, deserialize_vocab  # NOQA
import torch
from modelAOQ import BFAN, xattn_score
...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    max_n_word = 0
    for i, (images, captions, lengths, ids,_,_,_,_,_,_) in enumerate(data_loader):
...
def evalrank(model_path, model_path2, data_path=None, split='dev', fold5=False):
    ...
    if not fold5:
        ...
        sims2 = shard_xattn(img_embs2, cap_embs2, cap_lens2, opt2, shard_size=128)
        ...
        for i in range(5):
            ...
            start = time.time()
            sims2 = shard_xattn(img_embs_shard, cap_embs_shard, cap_lens_shard, opt2, shard_size=128)
            simsall = (sims + sims2)/2
